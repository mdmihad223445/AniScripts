-- =====================================================================
--  AniScript v8 — FIXED VERSION
--  ESP: Player(green) | Chest(gold) | Raft(blue) | Loot(brown) | Shark(red)
--  Auto Steal Raft Chests + Return Home After EACH Steal + Store + Repeat
--
--  BUGS FIXED vs v7:
--   [1] "tp and steal but won't tp base back"
--       -> returnToMySpot now finds your OWN raft dynamically (rafts drift,
--          so a saved CFrame becomes an ocean spot). savedCFrame is only
--          used as a last-resort fallback.
--   [2] "boxes on the sea count as chests"
--       -> isInsideRaft now uses raft:GetBoundingBox() (accurate) with a
--          tight +2 padding instead of the old +5 / 40-stud fallback that
--          swallowed nearby sea crates. Added a raycast check so the chest
--          must be physically resting on a raft. Chests must also have a
--          ProximityPrompt or ClickDetector (sea boxes don't).
--   [3] "after steal it goes to another chest without returning home"
--       -> New loop steals ONE chest, returns home, stores it, THEN goes
--          for the next chest. One chest per cycle.
--   [4] "won't tp to chest and steal again even if it's on"
--       -> The loop now never exits early. After returning home it waits
--          briefly and re-scans. A 'busy' guard prevents overlapping
--          cycles. autoSteal stays true until you turn it off.
-- =====================================================================
...
-- =====================================================================
--  10.  RAFT / CHEST HELPERS  (FIXED v8)
--  Major changes:
--   - isInsideRaft uses GetBoundingBox (accurate) + tight +2 padding,
--     no more 40-stud fallback that swallowed sea crates.
--   - isOnRaftSurface raycasts DOWN from the chest; the hit must belong
--     to the raft. Sea crates float over water -> excluded.
--   - hasInteraction requires a ProximityPrompt or ClickDetector. Real
--     chests have them, sea boxes don't.
-- =====================================================================
...
-- FIXED: accurate bounding box + tight padding. No more 40-stud fallback.
local function getRaftBoundingBox(raft)
    if not raft then return nil end
    if raft:IsA("Model") then
        local ok, cf, sz = pcall(function() return raft:GetBoundingBox() end)
        if ok and cf and sz then
            if sz.X > 0.5 and sz.Y > 0.5 and sz.Z > 0.5 then
                return cf, sz
            end
        end
        -- last resort: manual extents from parts (no fake 40-stud box)
...
local function isInsideRaft(part, raft)
    ...
    -- TIGHT padding: only +2 horizontal, +6 vertical (chests sit above floors)
    local half = size / 2 + Vector3.new(2, 6, 2)
...
-- NEW: raycast straight down from the chest. The hit part must belong to
-- the given raft. Floating sea crates raycast into water -> excluded.
local function isOnRaftSurface(part, raft)
    ...
    if part and part:IsDescendantOf(raft) then table.insert(filter, raft) end
    params.FilterDescendantsInstances = filter
    params.IgnoreWater = false
    local ray = workspace:Raycast(origin, Vector3.new(0, -80, 0), params)
    if not ray or not ray.Instance then return false end
    -- walk up the hit instance's ancestry; if we hit the raft, it's on it
...
-- NEW: real chests have a ProximityPrompt or ClickDetector. Sea crates don't.
...
-- =====================================================================
--  11.  AUTO STEAL  (FIXED v8 — one chest per cycle, always returns home)
-- =====================================================================
local autoSteal    = false
local stealing     = false       -- guard so cycles never overlap
local savedCFrame  = nil
local homeRaft     = nil         -- cached reference to my own raft
...
local function saveMySpot()
    -- cache the player's own raft so we can return to it even if it drifts
    homeRaft = findMyRaft()
    local _, hrp = getChar()
    if hrp then savedCFrame = hrp.CFrame end
end

-- continuously refresh saved spot + homeRaft when idle
task.spawn(function()
    while screenGui.Parent do
        task.wait(1)
        if not autoSteal and not stealing then saveMySpot() end
    end
end)

-- FIXED: now tries the player's OWN raft (dynamic position) first, so it
-- works even when the raft has drifted. savedCFrame is only a fallback.
local function returnToMySpot()
    local _, hrp = getChar()
    if not hrp then return false end

    -- 1) my own raft (re-find in case homeRaft got stale)
    local myRaft = homeRaft
    if not (myRaft and myRaft.Parent) then
        myRaft = findMyRaft()
        if myRaft then homeRaft = myRaft end
    end
    if myRaft and myRaft.Parent then
        local root = myRaft.PrimaryPart or myRaft:FindFirstChildWhichIsA("BasePart")
        if root then
    ...
    -- 2) saved CFrame (static, may be stale if raft drifted)
    if savedCFrame then
...
-- FIXED: stricter chest filtering — must be inside a raft, ON the raft
-- surface (raycast), AND have an interaction prompt. Kills sea boxes.
            ...
            if not Players:GetPlayerFromCharacter(obj) then
                if nameMatches(obj.Name, KEYWORDS.chests) then
                    ...
                    if root and root:IsA("BasePart") then
                        -- MUST have an interaction (sea crates don't)
                        if not hasInteraction(obj) then continue end
                        ...
                        -- MUST be physically resting on the raft
                        if not isOnRaftSurface(root, raft) then continue end
...
local function interact(entry)
    ...
    -- activate any held tool (in case the chest is picked up that way)
    ...
    -- fire raft prompts too (sometimes the steal trigger lives on the raft)
    if entry.raft then
...
-- NEW: store the chest at home by firing prompts on the home raft.
local function storeChestAtHome()
    ...
    -- fire any storage/deposit prompts on the home raft
...
-- =====================================================================
--  FIXED MAIN LOOP  —  steal ONE chest, return home, store, repeat.
--  The loop NEVER exits early. autoSteal stays true until you toggle it.
-- =====================================================================
task.spawn(function()
    while screenGui.Parent do
        task.wait(0.5)
        if autoSteal and not stealing then
            ...
            local ok, err = pcall(function()
                ...
                if #chests == 0 then
                    setStatus("Auto Steal: no valid raft chests found. Scanning...")
                    return
                end
                ...
                setStatus(("Stealing — %s (owner: %s) | 1 of %d")
                    :format(c.obj.Name, ownerName, #chests))

                -- STEP 1: teleport to chest and steal it
                stealChest(c)
                task.wait(0.3)

                -- STEP 2: ALWAYS return home (even if steal failed)
                setStatus("Returning home to store chest...")
                returnToMySpot()
                task.wait(0.2)
            ...
            if not ok then
                ...
                -- even on error, try to go home so we don't get stuck on enemy raft
...
stealBtn.MouseButton1Click:Connect(function()
    ...
    if autoSteal then
        saveMySpot()
        setStatus("Auto Steal: STARTED. Will steal -> return -> store -> repeat.")
    else
...
stopBtn.MouseButton1Click:Connect(function()
    ...
    returnToMySpot()
    setStatus("Stopped + returned home.")
end)
...
richestBtn.MouseButton1Click:Connect(function()
    local ok, err = pcall(function()
        ...
        setStatus(("Richest -> %s (value=%d, owner=%s)")
            :format(best.obj.Name, best.value, ownerName))
        print(("[AniScript Richest] %s | value=%d | owner=%s")
            :format(best.obj.Name, best.value, ownerName))
    end)
    if not ok then
...
setStatus("AniScript v8 ready. Fixed: returns home after EACH chest, ignores sea boxes.")
print("[AniScript v8] loaded. Bugs fixed:")
print("  [1] returnToMySpot now uses dynamic home-raft position (rafts drift)")
print("  [2] isInsideRaft uses GetBoundingBox + raycast + interaction check (no sea boxes)")
print("  [3] one chest per cycle, then return home + store, then next chest")
print("  [4] loop never exits early; autoSteal stays on; busy guard prevents overlap")
