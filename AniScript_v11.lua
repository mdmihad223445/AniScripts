-- =====================================================================
--  AniScript v11  --  PRO EDITION  (full rebuild of v10)
--  Game: raft-style games (Build and Defend Your Raft, etc.)
--
--  FIXED vs v10 (user report):
--   [1] AUTO STEAL: prompt firing rebuilt - line-of-sight is bypassed,
--       prompts are re-fired with RETRIES, nearby raft prompts are
--       included, and a touch-fallback jogs the chest for touch-based
--       games. Empty-chest check is now permissive (only skips chests
--       explicitly marked empty/opened/stolen). Chests standing on
--       furniture are recognised (raycast falls back to the raft box).
--   [2] GHOST MODE: completely rebuilt. You walk through WALLS only.
--       A downward raycast gives you real ground support, so you never
--       sink through floors anymore, and you never levitate - if there
--       is no floor below you, you simply fall and land normally.
--   [3] GOD MODE: heals every physics frame (not every 0.2s), blocks
--       death, joint-break and neck-kill, and keeps custom leaderstats
--       "Health"-style values maxed for games with their own HP bars.
--   [4] CHEST ESP: shows ONLY chests standing ON RAFTS. Floating sea
--       boxes / sea chests get their own "Sea Box ESP" (silver).
--   [5] SHARK ESP: sharks are now also found by their container folder
--       (workspace.Sharks / .Enemies / ...), every tag is always
--       visible (billboard + box outline), highlight budget respected.
--   [6] TP TO RICHEST CHEST: shielded rafts are detected much more
--       aggressively (attributes of any type, names, ForceFields,
--       shield parts inside the raft) and are NEVER teleported to.
--
--  NEW in v11:
--   + Walk on Water (MAIN tab, keybind V)
--   + Auto Collect Sea Loot
--   + Click TP (Ctrl + Click)
--   + Walk Speed / Jump Power boosters + reset
--   + Anti-AFK, FPS Boost
--   + Keybinds: RightCtrl UI, F Fly, G Ghost, V Water
--   + 4-tab animated PRO UI, one shared physics loop, cached scans
--
...
-- =====================================================================
--  1.  GUI SHELL  (compact PRO window: 336 x 404, 4 tabs)
-- =====================================================================
...
-- =====================================================================
--  2.  WIDGET FACTORIES  (animated toggles / sliders / buttons)
-- =====================================================================
local function makeHeader(parent, yPos, text)
    local bar = Instance.new("Frame")
    bar.Size = UDim2.new(0, 3, 0, 10)
    bar.Position = UDim2.new(0, 10, 0, yPos + 3)
    ...
    h.Position = UDim2.new(0, 18, 0, yPos)
...
local function makeSlider(parent, yPos, label, minV, maxV, initV, color)
    ...
    local track = Instance.new("Frame")
    track.Size = UDim2.new(1, -134, 0, 4)
    track.Position = UDim2.new(0, 86, 0.5, -2)
    track.BackgroundColor3 = THEME.track
    track.BorderSizePixel = 0
    track.Active = true
    track.Parent = row
...
local function makeButton(parent, yPos, text, bg, fg)
...
local function makeHint(parent, yPos, h, text)
    local t = Instance.new("TextLabel")
    t.Size = UDim2.new(1, -22, 0, h)
    t.Position = UDim2.new(0, 11, 0, yPos)
...
-- =====================================================================
--  3.  SCROLL AREA + TAB ROW + PAGES
-- =====================================================================
local scroll = Instance.new("ScrollingFrame")
scroll.Size = UDim2.new(1, -8, 1, -(TITLE_H + 30 + STATUS_H + 4))
scroll.Position = UDim2.new(0, 4, 0, TITLE_H + 30)
...
local mainPage = newPage(PAGE_H[1])
mainPage.Visible = true
local movePage = newPage(PAGE_H[2])
local espPage  = newPage(PAGE_H[3])
local miscPage = newPage(PAGE_H[4])
...
pill.Size = UDim2.new(0.25, -6, 0, 24)
pill.Position = UDim2.new(0, 3, 0, 1)
...
for i = 1, 4 do
    local b = Instance.new("TextButton")
    b.Size = UDim2.new(0.25, -6, 0, 24)
    b.Position = UDim2.new((i - 1) * 0.25, 3, 0, 1)
...
local function setTab(i)
    ...
    scroll.CanvasSize = UDim2.new(0, 0, 0, PAGE_H[i])
    TweenService:Create(pill,
        TweenInfo.new(0.18, Enum.EasingStyle.Quad, Enum.EasingDirection.Out),
        { Position = UDim2.new((i - 1) * 0.25, 3, 0, 1) }):Play()
    local p = pages[i]
...
statusLabel.Size = UDim2.new(1, -24, 1, -14)
statusLabel.Position = UDim2.new(0, 22, 0, 5)
...
-- ---------------- MAIN PAGE ----------------
makeHeader(mainPage, 2, "AUTO STEAL & FARM")

UI.steal = makeToggle(mainPage, 18, "Auto Steal Raft Chests", Color3.fromRGB(0, 255, 150))
UI.collect = makeToggle(mainPage, 48, "Auto Collect Sea Loot", Color3.fromRGB(255, 170, 60))

UI.richest = makeButton(mainPage, 78,
    "Teleport to Richest Chest",
    Color3.fromRGB(55, 45, 15),
    Color3.fromRGB(255, 220, 90))

UI.returnB = makeButton(mainPage, 106,
    "Return to My Raft",
    Color3.fromRGB(28, 40, 58),
    Color3.fromRGB(120, 200, 255))

UI.rescan = makeButton(mainPage, 134,
    "Scan World (F9 Report)",
    Color3.fromRGB(38, 38, 52),
    Color3.fromRGB(180, 180, 205))

makeHeader(mainPage, 166, "CHEATS")

UI.god = makeToggle(mainPage, 182, "God Mode", Color3.fromRGB(255, 80, 80))
UI.ghost = makeToggle(mainPage, 212, "Ghost Mode", Color3.fromRGB(180, 180, 255))
UI.water = makeToggle(mainPage, 242, "Walk on Water", Color3.fromRGB(56, 189, 248))
UI.fly = makeToggle(mainPage, 272, "Fly", Color3.fromRGB(120, 200, 255))

UI.flySpeed = makeSlider(mainPage, 302, "Fly Speed", 20, 300, flySpeed,
    Color3.fromRGB(120, 200, 255))

makeHint(mainPage, 332, 36,
    "Ghost: pass through walls only - floors still hold you, no floating.\n" ..
    "God: blocks all damage, death and stat drain.\n" ..
    "Water: step off your raft and walk across the sea (V).")

-- ---------------- MOVE PAGE ----------------
makeHeader(movePage, 2, "MOVEMENT")

UI.walkSpeed = makeSlider(movePage, 18, "Walk Speed", 16, 200, defaultWalk,
    Color3.fromRGB(34, 197, 94))

UI.jumpPower = makeSlider(movePage, 48, "Jump Power", 20, 300, defaultJumpPower,
    Color3.fromRGB(245, 158, 11))

UI.resetMove = makeButton(movePage, 78,
    "Reset Movement to Game Default",
    Color3.fromRGB(38, 38, 52),
    Color3.fromRGB(180, 180, 205))

UI.clickTp = makeToggle(movePage, 108, "Click TP (Ctrl + Click)", Color3.fromRGB(56, 189, 248))

makeHeader(movePage, 140, "KEYBINDS")

makeHint(movePage, 156, 48,
    "RightCtrl  -  show / hide UI\n" ..
    "F  -  toggle Fly          G  -  toggle Ghost\n" ..
    "V  -  toggle Walk on Water\n" ..
    "Ctrl + Click  -  teleport to cursor (when Click TP is ON)")

-- ---------------- ESP PAGE ----------------
makeHeader(espPage, 2, "ESP")

UI.espPlayers = makeToggle(espPage, 18, "Player ESP", Color3.fromRGB(0, 255, 120))
UI.espChests = makeToggle(espPage, 48, "Raft Chest ESP", Color3.fromRGB(255, 200, 0))
UI.espSeaboxes = makeToggle(espPage, 78, "Sea Box ESP", Color3.fromRGB(200, 205, 215))
UI.espRafts = makeToggle(espPage, 108, "Raft ESP", Color3.fromRGB(60, 140, 255))
UI.espLoots = makeToggle(espPage, 138, "Loot ESP", Color3.fromRGB(170, 110, 60))
UI.espSharks = makeToggle(espPage, 168, "Shark ESP", Color3.fromRGB(255, 50, 50))

makeHint(espPage, 198, 54,
    "Gold   = Raft Chest  (ONLY chests standing ON rafts)\n" ..
    "Silver = Sea Box     (boxes / chests floating on the sea)\n" ..
    "Blue   = Raft        ([SHIELD] = protected raft, skipped by steal)\n" ..
    "Brown  = Sea Loot    Red = Shark    Green = Player\n" ..
    "Every tag shows live distance in studs.")

-- ---------------- MISC PAGE ----------------
makeHeader(miscPage, 2, "UTILITY")

UI.antiAfk = makeToggle(miscPage, 18, "Anti-AFK (never get kicked)", Color3.fromRGB(34, 197, 94))
UI.fps = makeToggle(miscPage, 48, "FPS Boost", Color3.fromRGB(56, 189, 248))

makeHeader(miscPage, 80, "INFO")

makeHint(miscPage, 96, 68,
    "AniScript v11 PRO - every system rebuilt for speed and stability:\n" ..
    "one shared physics loop, cached world scans, flicker-free ESP,\n" ..
    "auto steal that hops raft-to-raft and skips shielded rafts.\n" ..
...
-- v11: PERMISSIVE empty check. A chest is only skipped when the game
-- explicitly marks it empty / opened / stolen. (v10's "must contain
-- resources" check rejected perfectly stealable chests -> the
-- "teleports but never steals" bug.)
...
-- =====================================================================
--  12.  SHIELD DETECTION v2  (the "tp to shield raft" fix)
--  A raft is shielded when ANY of these is true:
--    * a Shield/Protected/Locked attribute of ANY type is truthy
--      (true / "true" / "yes" / number > 0) on the raft or an ancestor
--    * the raft (or an ancestor) NAME contains shield/protected/...
--    * a ForceField or a "Shield"-named part/model lives inside it
-- =====================================================================
...
-- =====================================================================
--  15.  ESP ENGINE v2
--  * billboards (name + live distance) are ALWAYS created -> nothing
--    can ever be invisible, even past Roblox's 31-highlight limit
...
--  * sync is diff-based -> zero flicker
-- =====================================================================
...
local function refreshESP()
    ...
    -- RAFT CHESTS (ONLY chests standing ON rafts - the user's request)
    arr = {}
    local chestLabels = {}
...
-- =====================================================================
--  16.  CHEAT ENGINES  (fly / god / ghost / walk on water / movement)
...
-- ---------------- GOD MODE v2 ------------------------------------------
-- Layer 1: heal EVERY physics frame (v10 healed every 0.2s -> damage
--          still flashed through; now nothing is ever visible)
-- Layer 2: instant re-heal on the HealthChanged signal (same frame)
-- Layer 3: Dead state disabled + BreakJointsOnDeath + RequiresNeck off
-- Layer 4: games with a custom leaderstats "Health/Hp" bar are kept maxed
local godOn = false
...
local function hookGod(hum)
    ...
    godHookedHum = hum
    if (hum.MaxHealth or 100) > godMax then godMax = hum.MaxHealth end
    pcall(function()
        hum.MaxHealth = math.max(hum.MaxHealth or 100, godMax)
        hum.Health = godMax
    ...
    godHealthCon = hum:GetPropertyChangedSignal("Health"):Connect(function()
        ...
        if hum.Parent and hum.Health < godMax then
            pcall(function() hum.Health = godMax end)
        end
    end)
end
...
local function godTick()
    ...
    if hum ~= godHookedHum then
        ...
        pcall(function()
            if hum.MaxHealth > godMax then godMax = hum.MaxHealth end
            if hum.MaxHealth < godMax then hum.MaxHealth = godMax end
            if hum.Health < godMax then hum.Health = godMax end
        end)
    end
    godStatsHeal()
end
...
local function setGod(on)
    ...
    if godOn then
        ...
        setStatus("God Mode ON - damage, death and stat drain are all blocked.", "ok")
    else
        removeGod()
...
-- ---------------- GHOST MODE v2 (the floor / levitation fix) -----------
-- Noclip (pass through walls) + raycast ground support:
--   * floor under you            -> you stand on it, walk normally
--   * floor slightly below feet  -> smooth capped descent (stairs)
--   * no floor within range      -> you FALL normally (no levitating)
--   * jumping                    -> normal jump, land again on the floor
local ghostOn = false
...
-- =====================================================================
--  18.  PROMPT FIRING v2  (the "teleports but never steals" fix)
-- =====================================================================
-- v10 fired EVERY prompt on the raft - including the raft's own
-- "Drive / Steer / Sit" prompt - so you ended up DRIVING the enemy
-- raft instead of stealing the chest. These are filtered out now.
...
local function attemptSteal(entry)
    -- stand right on top of the chest (re-reads live position so moving
    -- / fleeing rafts are followed)
    if entry.part and entry.part.Parent then
        tpTo(entry.part.Position + Vector3.new(0, 2.5, 0))
    end
    task.wait(0.12)
    ...
    -- some games steal with the equipped tool
    local char = LocalPlayer.Character
    if char then
    ...
    -- touch-based chests get a physical jog
    if fired == 0 then
        touchJitter(entry)
    end
    return fired
...
local function stealChest(entry)
    ...
    for attempt = 1, 3 do
        ...
        local fired = attemptSteal(entry)
        task.wait(0.45)
        if not autoSteal then return false end
        ...
        if attempt < 3 then
            setStatus(("Stealing %s - attempt %d/3 (%d prompts fired)...")
...
-- =====================================================================
--  19.  AUTO STEAL LOOP  (raft -> raft forever, shield-safe)
-- =====================================================================
...
local function runStealCycle()
    ...
    setStatus(("Cycle %d -> %s (owner: %s) | %d chests queued")
...
task.spawn(function()
    while screenGui.Parent do
        task.wait(0.45)
        if collectOn and not collecting and not autoSteal then
            collecting = true
...
-- =====================================================================
--  21.  UTILITIES  (Anti-AFK / FPS Boost)
-- =====================================================================
...
-- =====================================================================
--  22.  WIRING  (buttons / sliders / keybinds)
-- =====================================================================
...
local function setSteal(on)
    ...
    if autoSteal then
        ...
        lastRaft = nil
        saveMySpot()
        setStatus("Auto Steal ON - hops raft to raft forever, skips shielded rafts. Toggle OFF to stop instantly.", "ok")
    else
...
UI.richest.MouseButton1Click:Connect(function()
    local ok, err = pcall(function()
        local res = getRaftChests(false, true)
        if #res.list == 0 then
            setStatus("Richest: no stealable raft chests (yours, crew, shielded and empty ones are skipped).", "warn")
            return
        end
        -- belt & braces: never teleport onto a shielded raft
        table.sort(res.list, function(a, b)
            local sa = isRaftShielded(a.raft) and 1 or 0
            local sb = isRaftShielded(b.raft) and 1 or 0
            if sa ~= sb then return sa < sb end
            if a.value ~= b.value then return a.value > b.value end
            return distTo(a.part) < distTo(b.part)
        end)
        local best = res.list[1]
        if isRaftShielded(best.raft) then
            setStatus("Richest: every chest found sits on a SHIELDED raft - nothing safe to teleport to.", "warn")
            return
        end
        if best.part and best.part.Parent then
            tpTo(best.part.Position + Vector3.new(0, 3, 0))
            setStatus(("Richest -> %s  (value %d, owner %s, %.0f studs away)")
                :format(best.obj.Name, best.value,
                        best.owner and best.owner.Name or "?", distTo(best.part)), "ok")
        else
            setStatus("Richest: that chest vanished mid-scan - try again.", "warn")
        end
    end)
    if not ok then
        warn("[AniScript] richest error:", err)
        setStatus("Richest error - see F9.", "err")
    end
end)
...
UI.rescan.MouseButton1Click:Connect(function()
    local ok, err = pcall(function()
        local res = getRaftChests(false, true)
        local s = res.stats
        print(("=== AniScript v11 CHEST SCAN: %d valid ==="):format(#res.list))
        ...
        for i, c in ipairs(res.list) do
            print(("[%d] %s | raft=%s | owner=%s | value=%d | dist=%.0f | shielded=%s"):format(
...
UI.espChests.btn.MouseButton1Click:Connect(function()
    ...
    setStatus(espState.chests
        and "Raft Chest ESP ON - ONLY chests standing ON RAFTS glow gold. Floating sea boxes are shown by Sea Box ESP."
...
-- =====================================================================
--  24.  BACKGROUND LOOPS  (all cheap, all gated)
-- =====================================================================
...
-- ESP refresh (diff-based, no flicker)
task.spawn(function()
    while screenGui.Parent do
        task.wait(1.5)
...
-- =====================================================================
--  25.  CLOSE (clean shutdown of every feature)
-- =====================================================================
...
--  26.  READY
-- =====================================================================
setStatus("AniScript v11 PRO ready. RightCtrl = show/hide. F = Fly, G = Ghost, V = Walk on Water.", "ok")

print("[AniScript v11 PRO] loaded.")
print("  FIXED: auto-steal fires prompts with retries + LOS bypass + touch fallback;")
print("  ghost mode walks through walls but never sinks through floors and never floats;")
print("  god mode heals every frame + blocks death/joint-break/stat drain;")
print("  chest ESP = raft chests ONLY (sea boxes have their own silver ESP);")
print("  shark ESP detects enemy folders too; TP-richest never targets shielded rafts.")
print("  NEW: walk on water (V), auto collect sea loot, click TP, speed/jump boost,")
print("  anti-AFK, FPS boost, 4-tab animated UI, single shared physics loop.")
